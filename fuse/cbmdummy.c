    ...
    #define PATH_GET_LAST_STATUS       "get_last_status.cmd"

    #define HANDLE_DUMMY_GET_CUR_STATUS     6
    #define PATH_GET_CUR_STATUS       "get_cur_status.cmd"
                                ...
                                struct fuse_file_info *fi)
    {
        ...
        if ((handle == HANDLE_DUMMY_GET_LAST_STATUS) ||
            (handle == HANDLE_DUMMY_GET_CUR_STATUS))
        {
            // TO DO need to check this will all fit in size!
            assert(size >= 32);
            DEBUG("Request to read drive status, size %zu offset %zu", size, offset);
            if (handle == HANDLE_DUMMY_GET_CUR_STATUS)
            {
                                ...
                                struct fuse_file_info *fi)
    {
        ...
        switch (handle)
        {
            ...
            case HANDLE_DUMMY_GET_CUR_STATUS:
